def send_email(message, recepient, *, sendler="university.help@gmail.com"):
    ...
    if recepient.find('@') == -1 or (recepient[len(recepient)-4:] != ".com"
            and recepient[len(recepient)-4:] != ".net"
            and recepient[len(recepient)-3:] != ".ru"):
        return print('Невозможно отправить письмо адресату ', recepient)

    elif sendler.find('@') == -1 or (sendler[len(sendler)-4:] != ".com"
            and  sendler[len(sendler)-4:] != ".net"
            and sendler[len(sendler)-3:] != ".ru"):
